...
public class GlassKilnMenu extends AbstractContainerMenu {
    ...
    public GlassKilnMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public GlassKilnMenu(int containerId, Inventory inv, BlockEntity blockEntity) {
        super(FPMenuTypes.GLASS_KILN_MENU.get(), containerId);
        this.blockEntity = ((GlassKilnBlockEntity) blockEntity);
    ...
    private static final int CRATE_INVENTORY_SLOT_COUNT = 12;
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, FPBlocks.GLASS_KILN.get());
    }
}
